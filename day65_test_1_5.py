...
#         if word in count:#This checks if the current word is already a key in the count dictionary.
#             count[word] += 1 #If the word is already in the dictionary, it means we've seen it before. So, count[word] += 1
#         else:
...
def string_manipulation(string):
    ...
    #for i in range(len(string)): "By doing this, our loop will go up to the last, but after the last, it won't find any character, which will cause it to throw an error."
    for i in range(len(string)-1): #A for loop starts, which goes through the characters in the input string one by one.
        if string[i] and string[i+1] or string[i+2]: # If the current character is the same as the next one, it means they are consecutive and repeating
